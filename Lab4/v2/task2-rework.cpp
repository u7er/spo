...
struct block {
    ...
    block* nextEmpty(){
	block *nxt;
        if(next != nullptr) nxt->next = this->next;

return nxt;
    }

	block* createNode(block *blk){
		block *tmp_blk = blk;
		block *t = this;
		if(head != nullptr){
			tmp_blk->head = this->head;
			tmp_blk->next = this->next;
...
static void* my_malloc_hook(size_t size, const void* caller)
{
    ...
    block *bp;
    for (bp = blk.head; bp.next != nullptr; bp.nextEmpty(), step++) {
        new_size = check_block(size);
        if (new_size == 0)
            return nullptr;
        if (bp->size >= new_size && bp->flag == 0) {
            block new_b;
            new_b.ptr = bp->ptr + new_size;
            new_b.size = bp->size - new_size;
            new_b.flag = 0;
            bp->size = new_size;
            bp->flag = 1;
            result = bp->ptr;
